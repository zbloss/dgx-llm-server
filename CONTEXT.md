...
### DFlash2 draft model
`z-lab/Qwen3.8-27B-DFlash2` - a ~1.9B-parameter block-diffusion speculative-decoding draft model for the target `Qwen/Qwen3.8-27B` checkpoint, downloaded to `/models/z-lab--Qwen3.8-27B-DFlash2` via the same GitOps sync as the main model. Adopted in ADR 0013, reverted in ADR 0015 (gives 0% prefix-cache hit rate on this model's hybrid GDN architecture, confirmed on every DFlash2-capable vLLM build tested, including the very first one). Still downloaded but currently unused (`method: mtp` is active) - kept for when ADR 0015's revisit condition (an upstream prefix-cache fix) lands.

### Qwen3.8-27B NVFP4
`unsloth/Qwen3.8-27B-NVFP4` - a native NVFP4 safetensors checkpoint (not GGUF). Released as a day-one NVFP4 quant, closing the gap that forced the prior llama.cpp-era stack (ADR 0005) onto GGUF K-quants for lack of a published NVFP4 GGUF. Ships a bundled `model_mtp.safetensors`, used via `--speculative-config '{"method":"mtp",...}'` (current, per ADR 0015).
